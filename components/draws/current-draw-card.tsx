...
export function CurrentDrawCard({
  ...
  return (
    <Card className="overflow-hidden border-sidebar-border bg-sidebar p-0 px-2 shadow-none">
      ...
      <CardContent className="grid gap-8 pb-8 md:grid-cols-2">
        {/* Left: Entry Numbers */}
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {[
              ...
              <div
                key={prize.label}
                className="flex items-center justify-between border-b border-sidebar-border/50 py-2 last:border-0"
              >
          ...
          <p className="text-[11px] text-muted-foreground italic">
            Note: Prize pool calculated from active subscriber count. Exact
            amounts published with draw results.
          </p>
        </div>
      </CardContent>

      <CardFooter className="border-t border-sidebar-border/50 bg-sidebar-accent/30 pb-4 text-xs text-muted-foreground">
        Draw runs on the 1st of each month. Your 5 latest scores automatically
        update your entry numbers when you add a new score.
      </CardFooter>
    </Card>
  )
}
