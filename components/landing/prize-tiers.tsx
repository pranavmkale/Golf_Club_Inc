...
function TierCard({ label, matchLabel, poolShare, description, isHero, note }: TierCardProps) {
  return (
    <Card
      className={[
        "relative flex flex-col border-border/50 transition-all duration-200 p-0",
        isHero
          ? "border-primary/40 bg-card shadow-lg shadow-primary/10 lg:scale-105 lg:z-10"
          : "bg-card/40 hover:-translate-y-1",
      ].join(" ")}
    >
      {isHero && (
        <div className="h-0.5 w-full rounded-t-xl bg-primary" />
      )}
      ...
      <CardContent className="flex flex-col gap-4 p-7">
        ...
        <div className="text-4xl font-black tracking-tight sm:text-5xl">{poolShare}</div>

        {/* Match label */}
        <p className="text-sm font-semibold text-muted-foreground">{matchLabel}</p>
    ...
    <section className="py-24 bg-muted/10 lg:px-24">
      <div className="container mx-auto px-6">
        ...
        <div className="mb-16 text-center">
          ...
          <p className="mt-3 text-muted-foreground">Monthly draws. Three ways to win.</p>
        </div>
        ...
        <p className="mt-10 text-center text-xs text-muted-foreground/70">
          Prize pool grows with every active subscriber. Prizes split equally among winners in the same tier.
        </p>
      </div>
    </section>
  )
