...
export async function POST(req: Request) {
  try {
    ...
    const { name, description, logo_url, cover_image_url, website_url, is_featured } = body

    if (!name || !description) {
      return NextResponse.json({ error: "Name and description are required" }, { status: 400 })
    }

    console.log("[Admin Charities] Creating charity:", { name, description, website_url, is_featured })
