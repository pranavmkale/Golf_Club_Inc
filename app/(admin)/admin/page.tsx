...
export default async function AdminOverviewPage() {
  ...
  const [
    ...
    supabaseAdmin
      .from("draws")
      .select(
        "id, draw_month, jackpot_amount, tier_4_amount, tier_3_amount, total_subscribers, status"
      )
      .order("draw_month", { ascending: false })
      .limit(10),
    supabaseAdmin
  ...
  const prizePool = prizePoolRes.data as {
    jackpot_amount: number
    tier_4_amount: number
    tier_3_amount: number
  } | null
  const latestPool = prizePool
    ? Number(prizePool.jackpot_amount) +
      Number(prizePool.tier_4_amount) +
      Number(prizePool.tier_3_amount)
    : 0
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Total users: {totalUsers} | Total prizes awarded: £
                {totalPrizeMoney.toLocaleString()} | Total draws: {draws.length}
              </p>
            </div>
